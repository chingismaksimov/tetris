    ...
    while True:
        ...
        if board.end_game():
            ...
            if action != None:
                ...
                print("The action taken is: %s" %(action))
                print("Change in reward is: %s" %(change_in_reward))
                print("Change in points is: %s" %(change_in_points))
                print("The predicted actions are: %s" %(predicted_actions))
                print("The updated actions are: %s" %(updated_actions))

                model.fit(state, updated_actions, epochs=10, batch_size=1)
        else:
            if action != None:
                new_state = convert_to_input(board, piece)
                new_points = board.refresh(points)
...
def create_model(learning_rate=0.005):
    model = Sequential()
    model.add(Conv2D(32, (1, 1), input_shape=(board_height, board_width, 1), strides=(1, 1), padding='same', data_format='channels_last', activation='linear'))
