    ...
    while True:
        ...
        if board.end_game():
            ...
            if action != None:
                ...
                updated_actions[action] = predicted_actions[action] + learning_rate_Q_update * (discount_factor * updated_actions[action] - losing_penalty - predicted_actions[action])
            ...
            if action != None:
                ...
                next_q_value = discount_factor * max(new_predicted_actions) + change_in_points + change_in_reward
                updated_actions = copy.deepcopy(predicted_actions)
                updated_actions[action] = predicted_actions[action] + learning_rate_Q_update * (next_q_value - predicted_actions[action])
                updated_actions = updated_actions.reshape(1, 5)
                print('#' * 330)
        ...
        next_piece.draw()
        board.draw()
        draw_text_on_next_piece(board_width * box_size + 150, 60)
        draw_text_on_points(board_width * box_size + 200, 160, points)
        draw_text_on_games_played(board_width * box_size + 200, 260, games_played)
