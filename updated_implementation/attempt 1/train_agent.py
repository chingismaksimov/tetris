...
def game():
    ...
    with open('hyperparameters.txt', 'w') as f:
        f.write('Starting exploration rate: {}'.format(start_exploration_rate))
    ...
    while True:
        ...
        if game_end == 0:
            target_action_values[0, action] = target_action_values[0, action] + step_size * (torch.max(updated_predictions) + points - initial_points - target_action_values[0, action])
        else:
        ...
        if len(X) % buffer_size == 0:
            ...
            if training_session % save_every == 0:
                with open('training_session_{}.txt'.format(training_session), 'w') as f:
                    ...
                    f.write('Lines cleared per game: {}'.format(total_lines_cleared / total_games_played))
...
def get_game_representation2(board, piece):
    ...
    board_column_heights = np.asarray(listed_board).reshape(board_height, board_width).sum(axis=0)

    piece_info = np.array([tetromino2int[piece.type], piece.rotation, piece.length, piece.height, piece.offset[0], piece.offset[1]])

    game_representation = np.concatenate((board_column_heights, piece_info))
...
class Agent(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(in_features=18,
                             out_features=64)
