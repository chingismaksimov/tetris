...
def game():
    ...
    while True:
        ...
        feature_vector, max_height = get_feature_vector(board, piece)
        ...
        if piece.at_bottom() or piece.collision(board, type='bottom'):
            if max_height > 0:
                points += max_height + piece.offset[1] - board_height
        ...
        updated_feature_vector, _ = get_feature_vector(board, piece)
...
def get_allowed_actions(board, piece):
    ...
    Returns
    _______
    allowed actions : numpy.ndarray with shape (5,) of allowed actions
    ...
    - rotating the piece clockwise is allowed
    - rotating the piece counterclockwise is allowed
    '''
    allowed_actions = [1]
...
def get_feature_vector(board, piece, normalize=True):
    ...
    column_heights = np.asarray(listed_board).reshape(board_height, board_width)
    column_heights = get_column_heights(column_heights)

    max_height = np.max(column_heights)

    bumpiness = np.abs(np.diff(column_heights)).sum().reshape(1)
    ...
    if normalize:
        ...
        normalized_bumpiness = (bumpiness - (board_width * board_height) / 2 + 0.5) / (board_width * board_height)
        normalized_piece_info = np.array([(tetromino2int[piece.type] - 3.5) / 7, (piece.rotation - 2.5) / 4, (piece.offset[0] - board_width / 2 + 0.5) / board_width, (piece.offset[1] - board_height / 2 + 0.5) / board_height])
        game_representation = np.concatenate((normalized_column_heights, normalized_bumpiness, normalized_piece_info))
    else:
        game_representation = np.concatenate((column_heights, bumpiness, piece_info))

    return game_representation, max_height
