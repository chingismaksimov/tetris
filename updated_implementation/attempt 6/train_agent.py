...
def get_game_representation2(board, piece):
    ...
    board_column_heights = (get_column_heights(board_column_heights) - board_height / 2 + 0.5) / board_height

    bumpiness = (np.abs(np.diff(board_column_heights)).sum().reshape(1) - 156 + 0.5) / 156

    piece_info = np.array([(tetromino2int[piece.type] - 3.5) / 7, (piece.rotation - 2.5) / 4, (piece.offset[0] - board_width / 2 + 0.5) / board_width, (piece.offset[1] - board_height / 2 + 0.5) / board_height])
