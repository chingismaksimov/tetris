...
def get_game_representation2(board, piece):
    ...
    board_column_heights = get_column_heights(board_column_heights) / board_height

    bumpiness = np.abs(np.diff(board_column_heights)).sum().reshape(1) / 100

    piece_info = np.array([tetromino2int[piece.type] / 7, piece.rotation / 4, piece.offset[0] / board_width, piece.offset[1] / board_height])
